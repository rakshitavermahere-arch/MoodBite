...
export default function OrderTracking() {
  ...
  const isDemo = Boolean(id?.startsWith("MB-DEMO"));

  // Demo-only: automatically step through the delivery stages every
  // couple of seconds, purely for a nicer live-feeling preview. Real
  // orders are unaffected and still reflect the actual backend status.
  const [demoStageIndex, setDemoStageIndex] = useState(0);
  useEffect(() => {
    if (!isDemo) return;
    if (demoStageIndex >= STAGES.length - 1) return;
    const timer = window.setTimeout(() => setDemoStageIndex((i) => i + 1), 2500);
    return () => window.clearTimeout(timer);
  }, [isDemo, demoStageIndex]);

  if (!order) return <PageWrap><p data-testid="tracking-order-not-found-alert">Order not found.</p></PageWrap>;
  const status = isDemo ? STAGES[demoStageIndex].key : String(order.status || "payment_pending").toLowerCase();
  const activeIndex = isDemo ? demoStageIndex : Math.max(0, STAGES.findIndex((stage) => stage.key === status));
  ...
  return (
    <PageWrap>
      <div className="max-w-2xl mx-auto">
        ...
        {status === "payment_pending" ? <div data-testid="tracking-payment-pending-alert" className="rounded-2xl border border-amber-300 bg-amber-50 p-5 text-amber-800">This order is waiting for verified payment and has not entered preparation.</div> : <div className="rounded-2xl border border-border bg-card p-6 sm:p-8 mb-6"><div className="space-y-7">{STAGES.map((stage, index) => { const done = index <= activeIndex; const current = index === activeIndex; return <div key={stage.key} className="flex items-center gap-4"><div className={`w-12 h-12 rounded-full grid place-items-center shrink-0 transition-colors duration-700 ${done ? "bg-eco text-eco-foreground scale-105" : "bg-muted text-muted-foreground"}`}><stage.icon className="w-5 h-5" /></div><div><p className={`font-heading font-bold ${done ? "text-foreground" : "text-muted-foreground"}`}>{stage.label}</p>{current && <p className="text-xs text-eco font-semibold">Current status</p>}</div></div>; })}</div></div>}
